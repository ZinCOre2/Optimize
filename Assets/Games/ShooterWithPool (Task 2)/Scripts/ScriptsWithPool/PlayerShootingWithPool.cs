...
public class PlayerShootingWithPool : MonoBehaviour
{
	...
	[Header("Bullets")] 
	public ObjectPoolType bulletType;
	...
	{
		GameObject bullet = ObjectPooler.Instance.GetObjectFromPool(bulletType);
	...
	void SpawnBulletSpread(Vector3 rotation)
	{
		...
		for (int x = min; x < max; x++)
		{
			...
			for (int y = min; y < max; y++)
			{
				...
				var bullet = ObjectPooler.Instance.GetObjectFromPool(bulletType);
